...
public class LocationDao {
    ...
    private String getLocationById = "select location_name from inmobiaclassified.location where location_id=?";
    ...
    public String getLocationById(int id) {
        ...
        try {
            ...
            con = DatabaseSource.getDatabaseConnection();
            logger.debug("Db connection made");
            pst = con.prepareStatement(getLocationById);
            pst.setInt(1, id);          
            ResultSet rs = pst.executeQuery();
            ...
            logger.debug("getLocationById completed");
        } catch (SQLException ex) {
            logger.error(ex.getMessage());
        } finally {
            try {
